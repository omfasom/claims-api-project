...
test.describe("Claims API – Negative / Validation Tests", () => {
    ...
    test("POST /claims – missing required field policyNumber", async () => {
    ...
    test("PATCH /claims/:id – invalid status transition (PAID → OPEN)", async () => {
    ...
    test("GET /claims/:id – claim not found", async () => {
        const response: any  = await client.get("/claims/NOT_EXISTING_ID");
        expect(response.code).toBe("CLAIM_NOT_FOUND");
    });

    test("PATCH /claims/:id – missing payoutAmount when APPROVED", async () => {
        const claim: any  = await client.post("/claims", {
